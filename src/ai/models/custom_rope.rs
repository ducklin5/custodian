...
impl CustomRotaryEncodingConfig {
    ...
    pub fn init<B: Backend>(&self, device: &Device<B>) -> CustomRotaryEncoding<B> {
        CustomRotaryEncoding::new(self.max_seq_len, self.head_dim, self.theta, self.attention_scaling, device)
    }
}

/// Custom rotary positional encoding that exactly matches HuggingFace Llama implementation
#[derive(Module, Debug)]
...
impl<B: Backend> CustomRotaryEncoding<B> {
    ...
    pub fn new(max_seq_len: usize, head_dim: usize, theta: f32, attention_scaling: f32, device: &Device<B>) -> Self {
        // Compute inv_freq: 1.0 / (theta^(2i/head_dim)) for i in [0, head_dim/2)
        ...
        let inv_freq = Tensor::<B, 1>::from_data(
            burn::tensor::TensorData::new(inv_freq_data, [half_dim]),
            device
        );
    ...
    /// Compute cos and sin values for given position IDs dynamically
    fn compute_cos_sin(&self, position_ids: &[usize], device: &Device<B>) -> (Tensor<B, 2>, Tensor<B, 2>) {
        let seq_len = position_ids.len();
        let half_dim = self.head_dim / 2;
        
        // Create position tensor from position_ids
        let positions_data: Vec<f32> = position_ids.iter().map(|&pos| pos as f32).collect();
        let positions = Tensor::<B, 1>::from_data(
            burn::tensor::TensorData::new(positions_data, [seq_len]),
            device
        );
        
        // Compute outer product: positions[i] * inv_freq[j] for all i, j
        let positions_expanded = positions.unsqueeze_dim::<2>(1); // [seq_len, 1]
        let inv_freq_expanded = self.inv_freq.clone().unsqueeze_dim::<2>(0); // [1, half_dim]
        let angles = positions_expanded * inv_freq_expanded; // [seq_len, half_dim]
    ...
    /// Apply rotary encoding to Q and K tensors exactly like HuggingFace
    /// 
    /// # Arguments
    ...
    pub fn apply_rope(
        ...
        let [_batch_size, _num_heads, seq_len, _head_dim] = q.dims();
        let device = q.device();
        ...
        // Compute cos and sin values dynamically
        let (cos_half, sin_half) = self.compute_cos_sin(&position_ids, &device);
        
        // Expand to full head dimension by repeating: [seq_len, half_dim] -> [seq_len, head_dim]
        let cos_full = Tensor::cat(vec![cos_half.clone(), cos_half], 1);
        let sin_full = Tensor::cat(vec![sin_half.clone(), sin_half], 1);
        
        // Apply rotary encoding
        let q_rot = self.rotate_tensor(q, &cos_full, &sin_full);
        let k_rot = self.rotate_tensor(k, &cos_full, &sin_full);
    ...
    fn rotate_tensor(
        ...
        let cos_expanded = cos_values.clone().reshape([1, 1, seq_len, head_dim]);
        let sin_expanded = sin_values.clone().reshape([1, 1, seq_len, head_dim]);
...
mod tests {
    ...
    fn test_custom_rotary_encoding() {
        ...
        let rope = CustomRotaryEncoding::new(max_seq_len, head_dim, theta, 1.0, &device);
