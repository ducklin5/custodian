...
pub fn qkv_attention_rotary<B: Backend>(
    q: Tensor<B, 3>,
    k: Tensor<B, 3>,
    v: Tensor<B, 3>,
    mask: Option<Tensor<B, 2>>,
...
pub fn repeat_kv<B: Backend>(x: Tensor<B, 4>, n_repeat: usize) -> Tensor<B, 4> {
    if n_repeat > 1 {
        let [n_batch, n_ctx, n_kv_head, n_hstate] = x.dims();
        x.repeat(&[1, 1, n_repeat, 1])
...
impl RotaryEncodingConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> RotaryEncoding<B> {
        ...
        let trig_vec = unsafe { vec![p_cos, p_sin] };
        let freq_cis = Tensor::cat(trig_vec, 1)
